...
| Интервал топота Пессимарио (§7.5) | 5.0 s **простоя** | Таймер тикает только в `idle` и стоит на время 0.7-секундного прыжка: период старт-к-старту ≈ 5.7 s |
...
| Круг приземления Макаки (§7.2) | y = 230 (временно) | §7.2 задаёт y=70 только точкам прыжка; в зоне босса урон недостижим. Финальное решение — [issue #2](https://github.com/LexorCrypto/FunGame/issues/2) |
...
Одинаковы в обеих фазах: фаза 2 сокращает окно (10.0 → 6.0 s) и умножает скорости
на 1.3, но каданс запуска атак внутри окна не трогает. Это отличие от боссов-владельцев
атак, у которых фаза 2 ускоряет и сам каданс.
