...
прогресс теряется, следующая атака идёт от нуля, призыв отродий готов сразу, а флаг
«плюх уже был в этом окне» тоже сбрасывается. Окно ротации при этом заново НЕ
начинается, поэтому если фаза сменится после первого плюха, в том же окне возможен
второй — расхождение со строкой «один раз за окно» ниже. Это известное поведение,
не решённое: [issue #10](https://github.com/LexorCrypto/FunGame/issues/10).

| Атака | Каданс в окне |
|---|---|
| Веер капель | каждые 2.0 s |
| Дуговые снаряды | каждые 3.0 s |
| Плюх с волной | один раз за окно |
