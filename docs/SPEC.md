...
Активные пауэр-апы — иконка (x 8, y 34): двойной выстрел — статичная иконка,
тройной — мигающая (§8, §11); таймера нет.
...
| Кулдаун | 250 ms | — |
| Макс. снарядов на поле | 4 на ствол — 4 / 8 / 12 по уровню оружия (§8) | 12 суммарно |
| Хитбокс снаряда | 3×6 | 4×4 |
| Урон | 1 | 1 попадание = смерть игрока |

Залп по уровню оружия (§8): одиночный — снаряд по центру; двойной — два
снаряда со смещением ±5 px по x; тройной — три со смещением −7 / 0 / +7 px.
Кулдаун один и тот же, отдельный «заряд» не тратится — уровень держится до
потери жизни (§8). Залп атомарен: если свободных снарядов в пуле меньше, чем
стволов, выстрела нет вовсе — иначе усиление вырождалось бы в неполный залп.
...
- **Интервал плюха — период старт-к-старту**, а не пауза между атаками: вся
  последовательность (телеграф 1.0 + падение 0.3 + возврат 2.0 = 3.3 s) лежит
  внутри периода, на простой остаётся 2.7 s в фазе 1 и 0.7 s в фазе 2. Решение
  владельца 2026-07-26 по [issue #1](https://github.com/LexorCrypto/FunGame/issues/1):
  раньше таймер тикал только в простое и период разъезжался до 9.3 / 7.3 s.
  Ровно так же считают период Супер-Туалет (§7.1), Макака (§7.2), Королева
  (§7.4), ключ и бур Пессимарио (§7.5) и все кадансы Трона (§7.6); единственное
  осознанное исключение — топот Пессимарио, см. §17.
...
| Бонус | Уровень | Эффект | Индикация |
|---|---|---|---|
| Оружие | 1 | 2 снаряда за выстрел | иконка в HUD |
| Оружие | 2 | 3 снаряда за выстрел | мигающая иконка в HUD (§1, §11) |
| Щит | 1 | поглощает 1 попадание | кольцо вокруг корабля, r=11 |
| Щит | 2 | поглощает 2 попадания | два кольца, r=11 и r=15, пульсация в противофазе: 450 ms в одну сторону, полный цикл 900 ms (`yoyo`); внешнее кольцо идёт 1.00 → 1.15, внутреннее в это же время 1.08 → 1.00 |

**Уровни** (решение владельца 2026-07-26). Каждый подобранный бонус поднимает
свой уровень на 1. Выше потолка (тройной выстрел / двойной щит) уровень не
растёт — такой бонус конвертируется в очки (§9). До этого решения оба бонуса
были одноуровневыми и повторный подбор пропадал впустую.

Выпадение: 30% шанс из убитого **пикирующего** врага и 10% с **каждого
попадания по боссу** (решение владельца 2026-07-26: в босс-волне пикировщиков
нет, и усилиться было негде). Распределение у обоих источников одно —
70% выстрел / 30% щит. Падает строго вниз со скоростью 60 px/s и **замирает в
зоне игрока** — y=223, середина полосы движения корабля (§1, §2), — где
подбирается касанием; висит 6.0 s, последние 1.5 s мигает, затем пропадает.
Точка появления по x зажата в диапазон корабля [8, 472]; появление ниже y=223
подтягивается к нему (бонус только падает, вверх не идёт). История ставки:
8% / 60-40 и сквозное падение за нижний край поля, из-за которого дроп был
недостижим → 20% / 70-30 и покой на y=223 (2026-07-25) → 30% (2026-07-26);
каждый раз по решению владельца.

Уровень оружия бессрочен: подобранные бонусы держатся до потери жизни (смерть
сбрасывает уровень в 0) и после респауна не возвращаются. Поглощение попадания
щитом жизнь не тратит, поэтому уровень оружия при этом сохраняется, а щит
теряет ровно один заряд. Таймера в HUD нет — только иконка (§1). До 2026-07-26
бонус жил 10.0 s с полоской-таймером; изменено по решению владельца.

## 9. Скоринг и рекорды

- Очки по таблице §4; пикирующий ×2; боссы — §7.
- **Попадание по боссу:** `+5` за каждый снаряд, попавший в босса (в цикле — ещё
  ×c, §6). Без всплывающего `+N`: при кулдауне 250 ms он появлялся бы до восьми
  раз в секунду с двойным выстрелом и до двенадцати с тройным (§3); обратная
  связь идёт счётчиком HUD, вспышкой босса и звуком `boss_hit`. Бонус за
  убийство босса (§7) — сверх этого.
- **Пауэр-ап на максимальном уровне:** `+500` за подобранный бонус, который уже
  не может поднять уровень (§8; в цикле — ещё ×c, §6). Значение выбрано между
  дорогим пикирующим врагом (туалет ×2 = 300) и бонусом чистой волны второго
  акта (500): подбор «в никуда» ощутимо платит, но добивание босса решает.
...
...KK..KK...
```
Пауэр-ап «Тройной выстрел» (12×7, 2 кадра — стволы мигают Y↔W, §1):
```
..KK.KK.KK..
.KYYKYYKYYK.
.KYYKYYKYYK.
...
| Интервал топота Пессимарио (§7.5) | 5.0 s **простоя** | Таймер тикает только в `idle` и стоит на время 0.7-секундного прыжка: период старт-к-старту ≈ 5.7 s в устойчивом режиме. На границе фаз он длиннее: вход во фазу 2 обнуляет таймер, а 1.0 s паузы замораживает уже начатый прыжок (≈6.7 s и более). Это единственная атака игры, считающая интервал от конца предыдущей: у остальных, включая плюх Супер-Какахи (§7.3, issue #1), интервал — период старт-к-старту |
