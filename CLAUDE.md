...
- **Откуда берутся значения указателей.** `main_sha` — ОПУБЛИКОВАННЫЙ коммит, а не локальный
  HEAD: `git fetch origin` → `git rev-list --left-right --count origin/main...HEAD` должен
  дать `0 0` → значение из `git rev-parse --short=7 origin/main` (полными SHA только сверяют,
  в поле идёт короткая форма — конвенция блока). Валидатор такое не ловит: `main_sha` он
  проверяет одной регуляркой `[0-9a-f]{7,40}` — git не вызывает вообще и достижимость
  коммита на remote не проверяет (его единственный внешний вызов, `gh issue view`, лишь
  читает тело в режиме `--issue`). Ровно так 2026-07-25 в тело попал `df3d924`, которого
  на `origin/main` не было.
