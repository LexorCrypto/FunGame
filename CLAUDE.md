...
- **Откуда берутся значения указателей, fail-closed.** `main_sha` — ОПУБЛИКОВАННЫЙ коммит, а
  ...
  Рефспек с destination обязателен: `git fetch origin refs/heads/main` пишет только
  `FETCH_HEAD`, tracking-ref обновляется в лучшем случае оппортунистически, и сверка ниже
  тогда сравнивает с устаревшим значением — ровно та ложная синхронность, от которой
  защищаемся. Fetch упал — процедура остановлена. После этого сверяются полные SHA:
